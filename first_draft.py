...
def main():
    ...
    with tabs[0]:  # All News Tab
                ...
                with col_login_signup[1]:
                    if st.form_submit_button("Sign Up"):
                        if not email or not password:
                            ...
                            error_message = _signup_user(email, password)  # Assuming _signup_user follows a similar pattern
                            if error_message:
                                st.sidebar.markdown(
                                    f"""
                                    <div style="width: 100%; padding: 10px; background-color: #a100ff; color: white; text-align: center; border-radius: 5px; margin-bottom: 10px;">
        ...
        display_articles(filtered_articles[:st.session_state.displayed_article_count], prefix="news")

        # Load More Button
        if len(filtered_articles) > st.session_state.displayed_article_count:
